...
mint() { # mint <id> <item_id> <origin>
    items "{\"id\":\"$1\"$(jt "$1"),\"action\":\"mint\",\"item_id\":\"$2\",\"origin\":\"$3\"}"
}
...
claim() { # claim <caller> <secret> <uid> <from> <to> <tick> <seq> <ws> [peer-tag]
    ...
    items "{\"id\":\"$1\"$(jt "$1"),\"action\":\"claim\",\"mid\":\"$MID\",\"uid\":\"$3\",\"from\":\"$4\",\"to\":\"$5\",\"tick\":$6,\"seq\":$7,\"ws_digest\":\"$8\",\"my_tag\":\"$mytag\"$extra}"
}
...
R=$(items "{\"id\":\"$ID1\"$(jt "$ID1"),\"action\":\"nope\"}")
...
R=$(items "{\"id\":\"$ID1\"$(jt "$ID1"),\"action\":\"list\"}")
expect "a player with no instances lists an empty wardrobe" '"items":[]' "$R"
R=$(items "{\"id\":\"$ID1\"$(jt "$ID1"),\"action\":\"seed\",\"items\":[\"cap\",\"scarf\",\"cap\",\"NOT AN ID\"]}")
...
R=$(items "{\"id\":\"$ID1\"$(jt "$ID1"),\"action\":\"seed\",\"items\":[\"jetpack\"]}")
...
R=$(items "{\"id\":\"$ID2\"$(jt "$ID2"),\"action\":\"list\"}")
...
R=$(items "{\"id\":\"$ID1\"$(jt "$ID1"),\"action\":\"claim\",\"mid\":\"$MID\",\"uid\":\"$U1\",\"from\":\"$ID1\",\"to\":\"$ID2\",\"tick\":101,\"seq\":1,\"ws_digest\":\"ws-101\",\"my_tag\":\"0123456789abcdef\"}")
expect "a claim the caller cannot attest to is refused" '"error":"bad self tag"' "$R"
R=$(items "{\"id\":\"$ID1\"$(jt "$ID1"),\"action\":\"claim\",\"mid\":\"$MID\",\"uid\":\"$U1\",\"from\":\"$ID1\",\"to\":\"$ID2\",\"tick\":101,\"seq\":1,\"ws_digest\":\"\",\"my_tag\":\"0123456789abcdef\"}")
expect "a claim with no ownership digest is malformed" '"error":"invalid claim"' "$R"
R=$(items "{\"id\":\"$ID1\"$(jt "$ID1"),\"action\":\"claim\",\"mid\":\"$NOTHING\",\"uid\":\"$U1\",\"from\":\"$ID1\",\"to\":\"$ID2\",\"tick\":101,\"seq\":1,\"ws_digest\":\"ws-101\",\"my_tag\":\"0123456789abcdef\"}")
...
if [ "$ADMIN" -eq 1 ]; then
    ...
    R=$(itemscode "{\"id\":\"$ID1\"$(jt "$ID1"),\"action\":\"mint\",\"item_id\":\"crown\",\"origin\":\"box\"}")
