...
hello_friends() { # id (the peer is whoever the delta names)
    curl -s -X POST -H 'Content-Type: application/json' \
        -d "{\"id\":\"$1\"$(jt "$1"),\"friends_since\":0}" "$BASE/api/hello.php"
}
...
curl -s -X POST -H 'Content-Type: application/json' \
    -d "{\"id\":\"$ID2\"$(jt "$ID2"),\"duel_end\":\"$ID1\"}" "$BASE/api/hello.php" > /dev/null
...
curl -s -X POST -H 'Content-Type: application/json' \
    -d "{\"id\":\"$ID2\"$(jt "$ID2"),\"duel_with\":\"$ID1\"}" "$BASE/api/hello.php" > /dev/null
R=$(hello_friends "$ID1" "$ID2")
expect "and a beat without the flag makes it public again" \
    "$(strict "\"$ID2\":{\"online\":true,\"playing\":true")" "$R"

R=$(curl -s -X POST -H 'Content-Type: application/json' \
    -d "{\"id\":\"$ID2\"$(jt "$ID2"),\"peer\":\"$ID1\",\"epoch\":0,\"reason\":\"first\",\"pts\":$(now_ms),\"duel_private\":\"yes\"}" \
    "$BASE/api/start.php")
...
R=$(curl -s -X POST -H 'Content-Type: application/json' \
    -d "{\"id\":\"$ID1\"$(jt "$ID1"),\"peer\":\"$ID2\",\"epoch\":3,\"reason\":\"first\"}" "$BASE/api/start.php")
...
R=$(curl -s -X POST -H 'Content-Type: application/json' \
    -d "{\"id\":\"$ID1\"$(jt "$ID1"),\"to\":\"$ID2\",\"type\":\"invite\",\"payload\":\"{}\",\"pts\":$FUTURE}" "$BASE/api/signal.php")
...
curl -s -X POST -H 'Content-Type: application/json' -d "{\"id\":\"$ID2\"$(jt "$ID2"),\"auto_accept\":true}" "$BASE/api/hello.php" > /dev/null
