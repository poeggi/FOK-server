...
final class Logs
{
    ...
    // so it reads as a warning here too, and FOK warning / FOK error are the
    // two levels a caller states outright; a plain "FOK <type>:" note stays
    // info, which is exactly what the note/raise split is for.
    private static function level(string $line): string
    {
        if (preg_match('/Fatal error|Parse error|Uncaught|FOK fault|FOK deferred|FOK error/i', $line) === 1) {
            return 'error';
        }
