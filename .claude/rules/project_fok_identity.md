...
## The owner's delete, and no device move (4.23)
...
takes them too; both need the strict proof. A device MOVE was built and
DROPPED before 1.20.0 shipped (decided 2026-09-19 on the client's side):
the shell mirrors id + tok into the platform keychain, which is backed
up and moves with the device, and a first run with an identity and no
config restores from the vault under it. The token proves the id from
any address, so nothing server-side is needed. Do not re-propose a
transfer code.
